function [Dh, Dl, timers] = train_coupled_dict_rj(Xh, Xl, dict_size, lambda, upscaleFactor)
% function [Dh, Dl, timers] = train_coupled_dict_rj(Xh, Xl, dict_size, lambda, upscaleFactor)
%
% Performs dictionary training on paired high- and low-resolution image
% patches (Xh, Xl, respectively).
%
% INPUTS:
%   Xh          = high-resolution image patches matrix (dims=[])
%   Xl          = low-resolution image patches matrix (dims=[])
%
% Called by: Demo_Dictionary_Training_rj.m 
