...
dictDir = 'newdicts';
nsizes = [100,50,25];
dictPatchsize = 5; overlap=dictPatchsize-1;
dictLam = 0.1; lambda=dictLam;
dsizes = [256,512,1024];
up_scale=3;
maxIter=20;
...
for nn=1:length(nsizes)
    nsamp=nsizes(nn);
    nndir=[dictDir filesep num2str(nsamp) 'k'];
    for dd=1:length(dsizes)
        ...
        dictName = strcat('D_',num2str(dictSize),'_lam-',...
            ...
            '_zoom-3_Yang.mat');
        dictNames{cnt}=fullfile(nndir,dictName);
        cnt=cnt+1;
        ...
        %save image
        fn_full_sr = fullfile(sprintf('New-Results-Testing/%s_res_sr.png',image_list{i}(1:end-4)));
        ...
        fmat = sprintf('New-Results-Testing/%s_results_SR.mat',image_list{i}(1:end-4));
