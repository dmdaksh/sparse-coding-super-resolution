function [U,S,V] = svd(X, econ, outputForm)
% SVD    Singular value decomposition.
% (Quaternion overloading of standard Matlab function.)

% Copyright © 2005,2023 Stephen J. Sangwine and Nicolas Le Bihan.
...
if nargin == 2
    ...
    % The second argument could be a value for econ or outputForm, so find
    % out which and adjust the parameters accordingly, assigning the
    % default value to outputForm if necessary. Leave the checking until
    % after this step, so that we can check parameters supplied with nargin
    % == 3.

    econ = convertCharsToStrings(econ);

    outputForm = "matrix"; % Supply the default value.

    if isstring(econ) && (econ == "vector" || econ == "matrix")
        outputForm = econ;
        econ = "";
    end
end
...
econ       = convertCharsToStrings(econ);
outputForm = convertCharsToStrings(outputForm);

% Now check the two parameters for sanity.

if ~isnumeric(econ) && ~isstring(econ)
    error(['Economy parameter must be numeric or string/char, found: ', ...
        class(econ)])
end

if isnumeric(econ) && econ ~= 0
    error(['Second parameter must be 0 if numeric, found: ', num2str(econ)])
end

if isstring(econ) && econ ~= "econ" && econ ~= ""
    error(['Second parameter must be ''econ'' if char/string, found: ', econ])
end

if isstring(outputForm)
    if outputForm ~= "vector" && outputForm ~= "matrix"
        error(['Acceptable values for outputForm parameter are: ', ...
               '''vector'' or ''matrix'', found: ', outputForm])
    end
else
    error(['outputForm parameter must be a string, found: ', ...
            class(outputForm)])
end

% Parameters are now checked.
...
if nargout == 0
    ...
    if nargin == 1
        svd(B)
    elseif isnumeric(econ) || (isstring(econ) && econ == "econ")
        svd(B, econ)
    else
        svd(B)
    end
...
elseif nargout == 1
    ...
    % In this case, the econ parameter may have been given, but if we were
    % to pass it to the Matlab svd routine, the returned U and V matrices
    ...
    [US, S, VS] = svd(B, outputForm);

    % ! S will now be a vector or matrix according to outputForm.
    ...
    if isnumeric(econ) || (isstring(econ) && econ == "econ")
        ...
        if isnumeric(econ) && econ == 0
            if m > n % If m <= n, there is nothing to do.
                ...
                if outputForm == "matrix"
                    S = S(1 : n, 1 : n);
                else
                    S = S(1 : n);
                end
            end
        elseif isstring(econ) && econ == "econ"
            if m == n
                return
...
% $Id: svd.m 1180 2023-10-10 16:42:08Z sangwine $
