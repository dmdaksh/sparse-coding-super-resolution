...
% Copyright © 2012, 2022 Stephen J. Sangwine and Nicolas Le Bihan.
...
% TODO Copy across code from @quaternion for the repeated squaring to an
% integer power. Octonions are power associative so this will work for
% single octonions.

% TODO Copy across code from Clifford, for the case of a vector of
% non-negative integer powers. Copy into @quaternion/power.m first, then to
% here should be a simple copy and paste.
...
if isscalar(Y) && isnumeric(Y)
    ...
    if Y == fix(Y)
        ...
        elseif Y == 0
            Z = octonion(ones(size(X), 'like', x(X.a)));
        elseif Y == 1
            Z = X;
        elseif Y == 2
            Z = X .* X;
            ...
            % associative). TODO We could use repeated squaring here
            % because of the power associativity. See the quaternion/power
