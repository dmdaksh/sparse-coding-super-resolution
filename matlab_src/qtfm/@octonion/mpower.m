...
if isscalar(Y) && isnumeric(Y) && isreal(Y) % Possibly case 2,
    if length(size(X)) == 2 && size(X, 1) == size(X, 2) % if X is square.
        ...
        % will build up.
        if Y < -1
            error(['Cannot handle negative powers. Octonion matrices '...
                   'have left and right inverses. See linv/rinv.'])
        elseif Y == -1
            error(['Octonion matrices have left and right inverses. '...
                   'See linv/rinv.'])
        elseif Y == 0
            Z = octonion(eye(size(X), 'like', x(X.a)));
            return;
        elseif Y == 1
            Z = X; return;
        elseif Y == 2
...
% $Id: mpower.m 1179 2023-10-10 16:41:06Z sangwine $
