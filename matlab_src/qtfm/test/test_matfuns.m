...
% Check that blkdiag works, at a simple level.

check(all(all(blkdiag(qi, qj, qk) == diag([qi, qj, qk]))), ...
    'blkdiag fails test');
...
% pinv - Pseudoinverse. Since release 3.5 of QTFM this is actually a QTFM
% function and not a Matlab function. One day maybe move this test
% elsewhere.

A = randq(4,5);
B = pinv(A);

compare(A * B, eyeq(4), T, 'pinv fails test 1')

A = A.';
B = pinv(A);

compare(B * A, eyeq(4), T, 'pinv fails test 2')
...
check(all(all(blkdiag(oi, ol, om, ok) == diag([oi, ol, om, ok]))), ...
    'blkdiag fails test');
...
% $Id: test_matfuns.m 1178 2023-10-10 16:37:52Z sangwine $
