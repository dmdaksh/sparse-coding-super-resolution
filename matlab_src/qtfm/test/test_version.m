...
V = ver('Matlab'); % This gets a structure containing information about the
        ...
        % TODO Replace ver above with isMATLABReleaseOlderThan and then
        % most of the code below can be done in one IF statement. This will
        % mean abandoning versions prior to R2020b. This would be OK if we
        % output only a warning, and use try/catch to handle the exception
        % caused if we are running an older version that doesn't have the
        % function.
if ~isempty(V)
...
% $Id: test_version.m 1178 2023-10-10 16:37:52Z sangwine $
