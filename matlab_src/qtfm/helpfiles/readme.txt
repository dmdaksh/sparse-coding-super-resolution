...
the XML files every time, from a list constructed in the file 'process.m'
by searching the directory for all files with .xml extension.
An easier way to run this script is to run the function qtfm_helpup which
is in the main QTFM folder, and therefore on the search path.
...
information. This can be done easily by running the function qtfm_helpdb,
which is located in the main QTFM folder, and is therefore on the search
path. The database is not distributed with the toolbox, because it is
specific to the version of Matlab that created it.
...
$Id: readme.txt 1173 2022-12-28 18:46:45Z sangwine $